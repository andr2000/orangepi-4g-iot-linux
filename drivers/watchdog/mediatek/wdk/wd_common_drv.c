...
#ifdef CONFIG_DEBUG
#define printk_deferred_if_debug	printk_deferred
#else
#define printk_deferred_if_debug(...)
#endif
	...
	for (;;) {
		...
		if (loc_wk_wdt && loc_wk_wdt->ready && g_enable) {
			...
			if (wk_tsk[cpu] != 0) {
				if (wk_tsk[cpu]->pid == current->pid) {
					...
					spin_lock(&lock);
					local_bit = kick_bit;
					printk_deferred_if_debug("[WDK], local_bit:0x%x, cpu:%d,RT[%lld]\n",
					...
					if (local_bit == wk_check_kick_bit()) {
						printk_deferred_if_debug("[WDK]: kick Ex WDT,RT[%lld]\n",
								sched_clock());
...
#ifdef CONFIG_LOCAL_WDT
					printk_deferred_if_debug("[WDK]: cpu:%d, kick local wdt,RT[%lld]\n",
							cpu, sched_clock());
					/* kick local wdt */
		...
		if ((cpu == 0) && (wk_tsk[cpu]->pid == current->pid)) {	/* only effect at cpu0 */
			if (aee_kernel_wdt_kick_api(g_kinterval) == WDT_PWK_HANG_FORCE_HWT) {
				printk_deferred_if_debug("power key trigger HWT\n");
